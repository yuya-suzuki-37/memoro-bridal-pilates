...
function analyzeSide(lms){
  ...
  // 頭部前方位の補助指標（巻き肩で肩が前方シフトしCVAが歪む場合用）: 耳が骨盤より前か
  const earFwdOfHip = forwardDir * (ear.x - hip.x) / trunkLen;
...
function detectProblems(sideRes, frontRes){
  ...
  // スウェイバックを先に判定（反り腰との排他のため）。
  // 「骨盤の大きな前方シフト＋上体の明確な後方」= 強いパターンのみを採用し、
  // 軽度の骨盤前方シフト（反り腰でも起こる）を誤ってスウェイバックにしない。
  const isSwayBack = m.hipFwdOfAnkle != null && m.hipFwdOfAnkle > 0.20 && m.shoulderBehindHip > 0.10;

  // 1. 頭部前方位（CVA < 50° = FHP / 小さいほど悪い）— _knowledge/01
  let hasForwardHead = false;
  if (m.cva != null && m.cva < 50) {
    const cva = m.cva;
    const severity = cva >= 45.5 ? 'low' : cva >= 40 ? 'mid' : 'high';
    problems.push(buildProblem('forwardHead', '頭部前方位（前方頭位）', severity, cva.toFixed(1)+'°', cva));
    hasForwardHead = true;
  }
  // 1b. 巻き肩で肩が前方シフトしCVAが過大評価される場合の補助判定（見逃し防止）。
  //     耳が骨盤基準より明確に前なら頭部前方位として拾う。
  if (!hasForwardHead && m.roundedShoulderRatio > 0.08 && m.earFwdOfHip != null && m.earFwdOfHip > 0.06) {
    const e = m.earFwdOfHip;
    const severity = e < 0.12 ? 'low' : e < 0.20 ? 'mid' : 'high';
    problems.push(buildProblem('forwardHead', '頭部前方位（前方頭位）', severity, '巻き肩に併発', e));
    hasForwardHead = true;
  }
  ...
  // 4. 骨盤前後傾（相対推定）— _knowledge/04（絶対角は断定しない）。スウェイバックとは排他。
  if (!isSwayBack && m.pelvicForward && m.pelvicTiltAngle > 8){
  ...
  } else if (!isSwayBack && !m.pelvicForward && m.pelvicTiltAngle > 8){
    const severity = m.pelvicTiltAngle < 12 ? 'low' : m.pelvicTiltAngle < 18 ? 'mid' : 'high';
    problems.push(buildProblem('posteriorPelvicTilt', '骨盤後傾傾向（相対推定）', severity, '後傾傾向', m.pelvicTiltAngle));
  }

  // 5. スウェイバック代理 — _knowledge/05（強いパターンのみ・反り腰と排他）
  if (isSwayBack) {
    const v = m.hipFwdOfAnkle;
    const severity = v < 0.28 ? 'low' : v < 0.38 ? 'mid' : 'high';
